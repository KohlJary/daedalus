"""
Terminal - A patched terminal widget with improved performance.

Based on textual-terminal with fixes for:
1. Batching output chunks instead of one-per-event
2. Debouncing screen rebuilds
3. Better async handling
4. OSC/DCS/APC escape sequence filtering

Copied from cass-vessel/tui-frontend/widgets/terminal_fast.py
"""
...
class Terminal(Widget, can_focus=True):
    ...
    # Keys that should pass through to the app (not captured by terminal)
    APP_PASSTHROUGH_KEYS = {
        "ctrl+f1",      # Release focus
        "ctrl+1",       # Focus Daedalus
        "ctrl+2",       # Focus Swarm
        "ctrl+3",       # Focus Lazygit
        "ctrl+q",       # Quit app
        "ctrl+n",       # New session (conflicts with terminal, but app needs it)
        "ctrl+k",       # Kill session (conflicts with terminal, but app needs it)
        "ctrl+w",       # Spawn worker (conflicts with terminal, but app needs it)
        "ctrl+x",       # Kill swarm (conflicts with terminal, but app needs it)
    }
    ...
    async def on_key(self, event: events.Key) -> None:
        ...
        # Let certain keys pass through to the app
        if event.key in self.APP_PASSTHROUGH_KEYS:
            return  # Don't stop, let it bubble up to app
